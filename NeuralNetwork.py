...
def backPropagation(X, y_n, s, weights):
    ...
    for layer in range(len(weights)-2,-1,-1):
        ...
        for nodei in range(1,len(weights[layer+1])):
            ...
            if width==1: #if there is only one node where the weights go (i.e. on the last iteration)
...
def test():
    ...
    plotErr(err,100)
    # print(forwardPropagation([1,2],[[[0.1,0.1],[0.2,0.2],[0.3,0.3]],[[0.4,0.4],[0.5,0.5]],[[0.6,0.6]]])[0])
...
# the confusion matrix from the scikit-learn implementation results in more correct predictions
# the error as the training of the NN progresses shows a decreasing trend
